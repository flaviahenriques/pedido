...
def carregar_imagem_base64(foto_obj):
    """
    Converte fotos (URL ou Upload) em Base64 para evitar 'ícone rasgado' no PDF.
    """
    if foto_obj.get('url_foto') and foto_obj['url_foto'].startswith("http"):
        try:
            ...
            if res.status_code == 200:
                ...
                ext = "jpeg" if "jpg" in foto_obj['url_foto'].lower() else "png"
    ...
    return "https://via.placeholder.com/400x300?text=Imagem+Indisponivel"
...
Compromisso de Agilidade: A PROFIX estabelece o prazo máximo de 48 horas para o atendimento e resolução de chamados após a abertura da solicitaçao.""",
...
Validade da Proposta: 30 dias."""
    ...
    with st.expander("3. Fotos e Valores", expanded=True):
        ...
        for idx, f in enumerate(st.session_state.fotos):
            ...
            img_p = f.get('url_foto') if f.get('url_foto') else f.get('file')
            if img_p: cc1.image(img_p, width=60)
