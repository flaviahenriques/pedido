...
def montar_layout_proposta(r_social, cnpj_val, empreend, local, cuidados, escopo, lista_itens, lista_fotos, valor_total):
    ...
    <body>
        <button class="no-print" onclick="window.print()">🖨️ IMPRIMIR OU SALVAR PDF</button>
        ...
        <div style="margin-top:30px; display:flex; justify-content:space-between; align-items:flex-start;">
            ...
            <div style="background:#f1f4f9; padding:20px; border-left:8px solid #002d5b; text-align:right; min-width:250px;">
                <small>TOTAL DA PROPOSTA</small><br><b style="font-size:26px; color:#002d5b;">R$ {valor_total:,.2f}</b>
            </div>
        </div>
    </body>
    </html>"""
...
if menu == "Gerenciar Pedidos":
    ...
    for p in pedidos:
        ...
        with st.expander(f"{p['cliente_razao_social']} - {p['empreendimento']}"):
            ...
            c1.write(f"**ID:** {p['id']} | **Valor:** R$ {float(p['valor_total']):,.2f}")
            if c2.button("📝 Editar", key=f"ed_{p['id']}"):
